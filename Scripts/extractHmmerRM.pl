...
# To extract and format the predictions from the output files of HMMER and RepeatMasker, excluding redundancies when 2 or more predictions are in the same region - the best one (with the lowest e-value OR the higher SWscore) is maintained in the analyses.
# Interesting for HMMER: the script excludes redundancies when using 2 or more pHMMs.

# Redundancy may occur when there is an overlapping region (total or partial) between any 2 PREDICTIONS; the redundancy is verified by (i) calculating (based on the "$maxPercOutOverlap" percentage) the maximum size ("$maxSizeOutOverlap") allowed for the region outside the overlapping region to consider that there is an overlap between the 2 predictions and also (ii) verifying for both predictions if the regions outside the overlapping region is smaller than a predefined value ("$maxRegionOutOverlap").
...
#				 FROM--fff---TO--ttt---LENGTH
	...
	for (my $j = $i+1; $j < $qttR; $j++) {
	    if ($sortedPredicts[$i]->{line} =~ /-->EXCLUD/) { last; } # it is correct at this point of the script because 		    "$sortedPredicts[$i]" may become "EXCLUD" when comparing with other predictions
	    if ($sortedPredicts[$j]->{line} !~ /-->EXCLUD/) {
		my $resp = verifyOverlap ($program, $sortedPredicts[$i]->{line},$sortedPredicts[$j]->{line});
...
sub verifyOverlap { # to verify overlap between 2 predictions and, if so, it retrieves an identification for the one to be disregarded; this prediction would be:
...
# $seq= PREDIC---FROM--ff---TO--tt---LENGTH--ll---RMSCORE--sc---SENSE--dr---MATCHINGREPEAT--match---TETYPE--type
    if (
	($seq1 =~ /^PREDIC---FROM--(\d+)---TO--(\d+)---LENGTH--(\d+)---EVALUE--(.*)---SCORE--.*---SENSE--(.*)---TETYPE/)    or
	($seq1 =~ /^PREDIC---FROM--(\d+)---TO--(\d+)---LENGTH--(\d+)---RMSCORE--(.*)---SENSE--(.*)---MATCHINGREPEAT/)
       ) {
	$from1    = $1;
	$to1      = $2;
	$length1  = $3;
    ...
    if (
	($seq2 =~ /^PREDIC---FROM--(\d+)---TO--(\d+)---LENGTH--(\d+)---EVALUE--(.*)---SCORE--.*---SENSE--(.*)---TETYPE/)    or
	($seq2 =~ /^PREDIC---FROM--(\d+)---TO--(\d+)---LENGTH--(\d+)---RMSCORE--(.*)---SENSE--(.*)---MATCHINGREPEAT/)
       ) {
	$from2    = $1;
	$to2      = $2;
	$length2  = $3;
    ...
    my $respOverl = "NONE"; # initially, it considers that there is NOT overlap between seq_1 and seq_2; otherwise, "$respOverl" retrieves an identification for the prediction to be disregarded
    if ($sense1 eq $sense2) {
	my $hasOverlap = "no";
#     testing if the predictions are apart from each other
	if ($to1 > $from2) {
#	  calculating the maximum size ("$maxSizeOutOverlap" - according to the prediction's length) outside the overlapping region 		  to consider that there is an overlap between the 2 predictions:
	   my $maxSizeOutOverlap1 = $maxPercOutOverlap * $length1;
	   my $maxSizeOutOverlap2 = $maxPercOutOverlap * $length2;
...
#	  testing the maximum allowed region outside the overlap
	   elsif ( ( ( ($from2-$from1) <= $maxSizeOutOverlap1) and ( ($from2-$from1) <= $maxRegionOutOverlap) ) or
		   ( ( ($to2-$to1)     <= $maxSizeOutOverlap2) and ( ($to2-$to1)     <= $maxRegionOutOverlap) )
