...
# To extract and format the predictions from the output file of RPS-Blast, excluding redundancies when 2 or more predictions of different domain types are in the same region - the best one (with the lowest e-value) is maintained in the analyses.
...
sub verifyOverlap { # to verify overlap between 2 predictions and, if so, it retrieves an identification for the one to be disregarded; this prediction would be the one with the highest e-value or, for the same e-values, the shortest prediction
