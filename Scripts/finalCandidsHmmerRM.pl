...
# $seq= Candidate_1 - FROM: ff - TO: tt - LENGTH: ll - RMSCORE: s - SENSE: dr - CLASSIFICATION: tetype\nPREDIC---FROM-- etc.
    if (
	($seq1 =~ /FROM: (\d+) - TO: (.*) - LENGTH: (.*) - (EVALUE|RMSCORE): (.*) - SENSE: (.*) - CLASSIFICATION/)
       ) {
	$from1    = $1;
	$to1      = $2;
	$length1  = $3;
    ...
    if (
	($seq2 =~ /FROM: (\d+) - TO: (.*) - LENGTH: (.*) - (EVALUE|RMSCORE): (.*) - SENSE: (.*) - CLASSIFICATION/)
       ) {
	$from2    = $1;
	$to2      = $2;
	$length2  = $3;
...
open (SAIDA, ">SAIDA.txt") or die "\nCan't open SAIDA.txt!!!\n\n";
if ($sortedCandidAllTEtype[$i] =~ /FROM--5499/) {
print SAIDA "$sortedCandidAllTEtype[$i]\n";
print SAIDA "$sortedCandidAllTEtype[$j]\n";
close SAIDA; 
