...
# This script uses 10 parameters, which can be changed in "ParamsGeneral.pm" package:
...
use ParamsGeneral qw(@superfamilies @tools @outFileNames @otherTools %outFileNamesOthers %filterTools %filterOtherTools %goodMetrTools %goodMetrOtherTools $distBetweenCands);
	...
	my @combClassif = (@superfamilies, "inconclusive");
	my $qttSpfam = scalar (@superfamilies);
	for (my $i = 0; $i < ($qttSpfam-1); $i++) {
		for (my $j = $i+1; $j < $qttSpfam; $j++) {
			my $combSpfam = $superfamilies[$i]."/".$superfamilies[$j];
			push (@combClassif, $combSpfam);
		}
	}
...
# OBS_1: IF RPS-BLAST IS USED IN THE SEARCHES FOR LTR-RT, DELETE THE COMMENTS ("#") IN FRONT OF THE FOLLOWING FIVE LINES:
...
# OBS_2 (b): IF A DIFFERENT TOOL IS CONSIDERED IN THE SEARCHES FOR LTR-RT, A NEW TEST CONDITION ("ELSIF") MUST BE INSERTED HERE; this new condition should filter the candidates based on the metrics used by the new tool.
			...
			foreach my $combSpfam (@combClassif) { if ($finalClass eq $combSpfam) { $combInCombClassif = "yes"; last;} }
			if ($combInCombClassif eq "no") {
				$typeClassFinal{$file}{$finalClass} = 0;
				push (@combClassif, $finalClass);
			}
