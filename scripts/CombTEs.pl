...
# However, it can treat TE candidates from other tools as well, since their final results (CANDIDATEs) are in the format:
# CANDIDATE_1 - FROM: pos_INIT - TO: pos_Final - LENGTH: length - SENSE: Direct/Reverse - CLASSIFICATION: superfam_A
# PREDIC---FROM--pos_init---TO--anyPos---LENGTH--length_1---METRICS--metr---SENSE--D/R---SUPERFAM--spfam_A
# PREDIC---FROM--anyPos---TO--anyPos---LENGTH--length_2---METRICS--metr---SENSE--D/R---SUPERFAM--spfam_A
# ...
# PREDIC---FROM--anyPos---TO--pos_final---LENGTH--length_N---METRICS--metr---SENSE--D/R---SUPERFAM--spfam_A
# (one blank line)
...
# The predictions ("PREDIC") that make up each "CANDIDATE" must be sorted by their "FROM" positions; then, the FROM ("pos_init") position of the first prediction of a CANDIDATE will be the FROM ("pos_INIT") position of the CANDIDATE and the TO ("pos_final") position of the last prediction of the CANDIDATE will be the TO ("pos_FINAL") position of the CANDIDATE.
...
# If a different tool is used to search for LTR-RT, a new "test condition" must be inserted at ... (search for "OBS_2" below).
...
# TODO: dar uma jeito em @combClassif ? ???????????????????????????????????????????????????????
...
# WHEN using RPS-Blast: CombTEs assumes that "extractRPSB.pl" was used to extract and format the predictions of RPS-Blast and its output file, with the conserved domain, has the name "ConservedDomains_NoRedund.pred", to be used by "finalCandidsRpsBlast.pl":
		elsif ($tool eq "RpsBlast") { @candidSpfam = `perl finalCandidsRpsBlast.pl`; }

# WHEN a different tool is used to search for LTR-RT, the user must provide the correspondent script to process the initial candidates of this new tool; the name of the new script must replace "scriptOtherTOOL" below. (DELETE comment in the next line):
...
# OBS_2: IF A DIFFERENT TOOL IS CONSIDERED IN THE SEARCHES FOR LTR-RT, A NEW TEST CONDITION ("ELSIF") MUST BE INSERTED HERE; this new condition should filter the candidates based on the metrics used by the new tool.
